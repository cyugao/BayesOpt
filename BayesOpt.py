...
class GPBO(object):
    def __init__(
        ...
        if acquistion_fn == "EI":
            ...
            self.acquistion_fn = self.thompson
        self.f = f
    ...
    def thompson(self, x):
        ...
        return mu + variance.sqrt() * pyro.distributions.Normal(0, 1).rsample()
    ...
    def next_x(self, lower_bound=0, upper_bound=1, num_candidates=5):
        ...
        for i in range(num_candidates):
            ...
            candidates.append(x)
            values.append(y)
            x_init = x.new_empty((1, self.input_dim)).uniform_(lower_bound, upper_bound)
    ...
    def train(self, iter=8):
        plt.figure(figsize=(12, 30))
        outer_gs = gridspec.GridSpec(5, 2)
        optimizer = torch.optim.Adam(self.gpmodel.parameters(), lr=0.001)
        gp.util.train(self.gpmodel, optimizer)
        for i in range(iter):
            xmin = self.next_x(self.lower_bound, self.upper_bound, self.num_candidates)
